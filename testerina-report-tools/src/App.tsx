...
import { TestData, Test, WorkspaceData, normalizeToWorkspaceData } from './TestData';
...
let rawData: any = JSON.parse(jsonContainer.innerHTML);
...
class App extends Component {
  ...
  constructor(props: any) {
    super(props);
    this.state = { view: "index", projectIndex: 0, moduleIndex: 0, fileIndex: 0, selectedTest: null }
    this.handleStateChange = this.handleStateChange.bind(this);
    this.handleEvalClick = this.handleEvalClick.bind(this);
  }
  ...
  handleEvalClick(test: Test) {
    this.setState({ view: "eval", selectedTest: test });
  }
  ...
  getModuleTableView(testData: TestData, moduleIndex: number, isCoverageAvailable: boolean) {
    ...
    return <div className="module-content">
      ...
      <div className="row">
        <ModuleStatusSummary module={testData.moduleStatus[moduleIndex]} onEvalClick={this.handleEvalClick} />
        {moduleCoverageHtml}
      </div>
    </div>
  }
  ...
  render() {
    ...
    if (this.state.view.includes("index")) {
      ...
      summaryView = <FileCoverage
        ...
        updateState={this.handleStateChange}
      />
    } else if (this.state.view.includes("eval") && this.state.selectedTest) {
      tableView = null
      summaryView = <EvalPage
        test={this.state.selectedTest}
        moduleName={currentProject.moduleStatus[this.state.moduleIndex].name}
        onBack={() => this.handleStateChange("module", this.state.moduleIndex, 0, this.state.projectIndex)}
      />
    }
    return (
